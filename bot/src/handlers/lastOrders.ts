// Пример команды /last_orders — только для админа: отправляет список последних заказов (упрощённо)
import { Context } from "telegraf";
import fetch from "node-fetch";

export async function handleLastOrders(ctx: Context, adminChatId: string) {
  if (String(ctx.chat?.id) !== adminChatId) return; // Только для админ-чата!
  try {
